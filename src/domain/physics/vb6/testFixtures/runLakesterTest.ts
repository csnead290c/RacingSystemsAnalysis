/**
 * Run Bonneville Pro 3.2 Lakester Test Case
 * 
 * This script runs the lakester test case and compares results
...
import { 
  BONNEVILLE_LAKESTER, 
  BONNEVILLE_LAKESTER_EXPECTED,
  BONNEVILLE_LAKESTER_CHECKPOINTS 
} from './bonnevilleLakester';

// Run the test
console.log('='.repeat(70));
console.log('Bonneville Pro 3.2 Test Case - lakester.dat');
console.log('='.repeat(70));

// Convert fixture to sim inputs
// VB6 shows "Track: Bonneville - 3 Miles" which is 15840 ft
const simInputs = fixtureToSimInputs(BONNEVILLE_LAKESTER, 'BONNEVILLE_SHORT');

// Calculate air density for display
const airResult = airDensityVB6({
  barometer_inHg: BONNEVILLE_LAKESTER.env.barometer_inHg,
  temperature_F: BONNEVILLE_LAKESTER.env.temperature_F,
  relHumidity_pct: BONNEVILLE_LAKESTER.env.relHumidity_pct,
  elevation_ft: BONNEVILLE_LAKESTER.env.elevation_ft,
  fuelSystem: 5, // Supercharged Methanol
});

console.log('\nInput Summary:');
console.log(`  Weight: ${simInputs.vehicle.weightLb} lb`);
console.log(`  Power: ${BONNEVILLE_LAKESTER.engineHP[4][1]} HP peak`);
...
console.log(`  Traction Index: ${BONNEVILLE_LAKESTER.env.tractionIndex}`);
...
console.log('\nFinal Results:');
console.log(`  VB6 Expected: ${BONNEVILLE_LAKESTER_EXPECTED.finalTime_s}s @ ${BONNEVILLE_LAKESTER_EXPECTED.finalSpeed_mph} MPH @ ${BONNEVILLE_LAKESTER_EXPECTED.finalRPM} RPM`);
console.log(`  Our Result:   ${finalTime.toFixed(2)}s @ ${finalSpeed.toFixed(1)} MPH @ ${finalRPM.toFixed(0)} RPM`);
console.log(`  Time Error:   ${(finalTime - BONNEVILLE_LAKESTER_EXPECTED.finalTime_s).toFixed(2)}s`);
console.log(`  Speed Error:  ${(finalSpeed - BONNEVILLE_LAKESTER_EXPECTED.finalSpeed_mph).toFixed(1)} MPH`);
console.log(`  RPM Error:    ${(finalRPM - BONNEVILLE_LAKESTER_EXPECTED.finalRPM).toFixed(0)} RPM`);
...
for (const checkpoint of BONNEVILLE_LAKESTER_CHECKPOINTS) {
...
// HP Analysis
console.log('\n' + '-'.repeat(70));
console.log('HP Analysis (every 5 seconds):');
console.log('-'.repeat(70));
console.log('Time(s)   MPH      HP       DragHP   NetHP    Accel(g)');
console.log('-'.repeat(70));

let lastHPPrintTime = -5;
